...
# ################################---------------------------------------------
# Translations from Source data structure to Entity data structure
# ################################

    # Data from source enters
    # data is processed for each type
    # Entities, attributes, and relationships are created using the schema defined on input
    # Data is then exported as  fully defined entity in hash format. 
    # (maybe it would be clean to create a way to have them sit next to each other)
...
# Object holding all the entities

# At this point every entity and its relationship to other entities is defined.
# Triples can be generated directly from this and then upload on to the system.